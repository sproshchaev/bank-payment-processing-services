...
// @RequestMapping(value = "/authorization") - @RequestMapping annotation not allowed on @FeignClient interfaces
public interface AuthorizationServiceProxy {

    @GetMapping(value = "/tid/{tid}/date/{date}/card/{card}/expdate/{expdate}/sum/{sum}/curr/{curr}")
    public PaymentValueObject paymentAuthorization(@PathVariable String tid, @PathVariable String date, @PathVariable String card,
